...
def evaluate_agent(agent, env, device, num_episodes=1, short_ae=False):
    agent.eval()
    total_rewards = []
...
def evolutionary_phase(envs, population_size, generations, sigma, device, base_agent, num_episodes=1, short_ae=False):
    # Generujemy początkową populację: kopiujemy bazowy model i stosujemy mutację
    population = []
    new_population = []
    for _ in range(population_size):
        agent = DQN(envs[0].action_space.n).to(device)
    ...
    for gen in range(generations):
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            futures = [executor.submit(evaluate_agent, agent, envs[i], device, num_episodes=num_episodes, short_ae=short_ae) for i, agent in enumerate(population)]

            for future in concurrent.futures.as_completed(futures):
                avg_reward, traj = future.result()
        ...
        while len(new_population) < population_size:
            ...
            child = DQN(envs[0].action_space.n).to(device)
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = [executor.submit(evaluate_agent, agent, envs[i], device, num_episodes=num_episodes) for i, agent in enumerate(population)]
...
def deep_Q_learning(env, folder, batch_size=32, M=50_000_000, epsilon_start=1, epsilon_end=0.1, nb_exploration_steps=1_000_000,
                    buffer_size=1_000_000, gamma=0.99, training_start_it=80_000, update_frequency=4, device='cuda', C=10_000, ae_steps=100_000,
                    population_size=5, generations=3, sigma=0.01, ae_start_after=0, best_to_new_model=False, game_name=None, short_ae=False):
    ...
    envs = []
    for _ in range(population_size):
        env_tmp = gym.make(game_name) # 'BreakoutNoFrameskip-v4'
        env_tmp = gym.wrappers.ResizeObservation(env_tmp, (84, 84))
        env_tmp = gym.wrappers.GrayscaleObservation(env_tmp)
        env_tmp = gym.wrappers.FrameStackObservation(env_tmp, stack_size=4)
        env_tmp = MaxAndSkipEnv(env_tmp, skip=4)
        envs.append(env_tmp)
    ...
    while epoch < M:
        ...
        if ae_explore and population_size > 1 and epoch > ae_start_after:
            best_agents, evo_experiences, mean_reward_generation, best_reward_generation = evolutionary_phase(envs, population_size, generations, sigma, device, q_network, num_episodes=1, short_ae=short_ae)
...
def experimentFull(game_name, short_ae=False):

    folder = f'gradient_ae_experiments_full/{game_name}'
    os.makedirs(f'results/{folder}/models', exist_ok=True)
    os.makedirs(f'results/{folder}/logs', exist_ok=True)

    env = gym.make(game_name) # 'BreakoutNoFrameskip-v4'
    ...
    q_network, mean_reward_generations, best_reward_generations, experiance_from_evo, all_rewards = deep_Q_learning(
        env, folder, batch_size=32, M=50_000_000, epsilon_start=1, epsilon_end=0.01, nb_exploration_steps=1_000_000,
        buffer_size=1_000_000, gamma=0.99, training_start_it=80_000, update_frequency=4, device='cuda', C=10_000, ae_steps=100_000,
        population_size=8, generations=3, sigma=0.0005, ae_start_after=0, best_to_new_model=False, game_name=game_name, short_ae=short_ae
    )
...
if __name__ == "__main__":
    ...
    game_names = ['AtlantisNoFrameskip-v4', 'SeaquestNoFrameskip-v4', 'AsteroidsNoFrameskip-v4', 'BreakoutNoFrameskip-v4', 'FrostbiteNoFrameskip-v4']

    parser = argparse.ArgumentParser()
    parser.add_argument("--game_index", type=int, default=0)
    parser.add_argument("--short_ae", type=bool, default=False)

    args = parser.parse_args()

    game_index = args.game_index
    game_name = game_names[game_index]
    short_ae = args.short_ae

    print(f"Starting experiment for {game_name} with short_ae: {short_ae}")
    print("Starting for 5 seconds")
    time.sleep(5)

    experimentFull(game_name, short_ae)
